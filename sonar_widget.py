...
class SonarViewer(pyqtWidget.QDialog):
    ...
    def FDisplayImage(self, ffigure = None):
        ...
        frames = self.playback_manager.sonar.FRAMES
        ...
        if(self.subtractBackground):
            ...
            if(self.marker):
                cv2.circle(frames, literal_eval(self.marker), 30, (255,255,255), 1)
                cv2.circle(mask, literal_eval(self.marker), 30, (255,255,255), 1)
                
            img = np.hstack((mask, frames))
            img = pyqtGUI.QImage(img, img.shape[1], img.shape[0], img.strides[0], qformat)
        
        else:
            img = pyqtGUI.QImage(frames, frames.shape[1], frames.shape[0], frames.strides[0], qformat)
        ...
        #self.FParent.FStatusBarFrameNumber.setText("Frame : "+str(self.UI_FRAME_INDEX+1)+"/"+str(self.playback_manager.sonar.frameCount))
